...
public static class WebExporter
{
    ...
    {
        string destFile = Path.Combine(settings.DestFolder, env.RootFilename + "-viewer.html");
    ...
    private static string GenerateViewDoc(string jsonContent, string rootFilename, bool locActions)
    {
        return @$"
<!DOCTYPE html>
<html>
<head>
    <title>Dink Viewer: {rootFilename}</title>
    <style>{GetCss()}</style>
</head>
<body>
    <header class=""main-header"">
        <h1>Dink Viewer: {rootFilename}</h1>
    ...
    <script>{GetJavascript(locActions)}</script>
</body>
</html>
";
    }
